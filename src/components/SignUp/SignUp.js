...
const FormContainer = styled.div`
  ...
  ${tabletPo({ width: "40vw" })};
  ${desktop({ height: "55vh" })};
  ${large({ width: "30vw", height: "60vh", margin: "5px" })}
`;
...
const SignUp = () => {
  ...
  const { name, email, mobile, password, passwordConfirm } = values;
  ...
  const createUser = async (values) => {
    const { uname, uemail, umobile, upassword, upasswordConfirm, users } =
      userValues;
    let flag = true;
